...
class ConsolidateGeneralGradesStudyYear implements FromArray, WithColumnWidths, WithStyles, WithEvents
{
    private $studyYear;
    private $Y;
    private $ST;
    ...
    public function __construct($Y, $studyYear, $ST, $period, $areas, $students)
    {
        $this->Y = $Y;
        ...
        $periods = \App\Models\Period::where('school_year_id', $Y->id)->where('study_time_id', $ST->id)
            ->when(
                $period !== 'FINAL', fn($whenNoFinal) => $whenNoFinal->where('ordering', '<=', $period->ordering)
    ...
    private function calculateGradeWorkload($grade, $ac) { return $grade ? ($grade ?: $this->ST->minimum_grade) * $ac : null; }
    ...
    public function array(): array
    {
        ...
        if ($this->period === 'FINAL') { $periodsAfter = []; }
            ...
            $periodsAfter = Period::where('school_year_id', $this->Y->id)->where('study_time_id', $this->ST->id)->where('ordering', '>', $this->period->ordering)->get();
        }
        $minimalGrade = ($this->ST->low_performance + $this->ST->step);
        ...
        $array = [
            ...
            [__('export.study_time') . ': ' . $this->ST->name . ' | ' . __('export.study_year') . ': ' . $this->studyYear->name],
            [$infoPeriods],
            $headers
        ];
        ...
        foreach ($this->students as $student) {
            ...
            foreach ($this->areas as $area) {
                ...
                foreach ($area['subjects'] as $subject) {
                    ...
                    $tsg = collect($subject['tsg'])->filter(function ($filterTsg) use ($student) {
                        return $filterTsg['group_id'] === $student->groupYear->group_id;
                    })->first() ?? collect();

                    $existStudentLeveling = !is_null($tsg['id'])
                        ? \App\Models\LeveledStudent::where('teacher_subject_group_id', $tsg['id'])->where('student_id', $student->id)->count()
                        : 0;

                    foreach ($this->periods as $period) {

                        $gradeByStudentByPeriod = $student->grades->filter(function ($filterGrade) use ($tsg, $period) {
                            return $filterGrade->teacher_subject_group_id === $tsg['id'] && $filterGrade->period_id === $period->id;
                        })->first();
            ...
            foreach ($this->areas as $area) {
                ...
                foreach ($area['subjects'] as $keySubject => $subject) {
                    ...
                    foreach ($this->periods as $period) {
                        ...
                        $sumArea += $this->calculateGradeWorkload($gradeByStudentByPeriod?->final, $subject['academic_wordload_porcentage']);
                        ...
                        $totalSubject += $this->calculateGradeWorkload($gradeByStudentByPeriod?->final, $subject['academic_wordload_porcentage']);
