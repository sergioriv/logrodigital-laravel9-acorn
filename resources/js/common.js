...
class Common {
    ...
    _initTooltips() {
        ...
        var tooltipTriggerListNoDelay = [].slice.call(
            document.querySelectorAll(
                '.no-delay[data-bs-toggle="tooltip"][data-bs-delay="0"]'
            )
        );
        var tooltipListNoDelay = tooltipTriggerListNoDelay.map(function (
            tooltipTriggerElNoDelay
        ) {
    ...
    _initScrolls() {
        if (typeof OverlayScrollbars !== "undefined") {
            ...
            OverlayScrollbars(
                document.querySelectorAll(".data-table-rows .table-container"),
                {
                    overflowBehavior: { x: "scroll", y: "hidden" },
                }
            );
            OverlayScrollbars(
                document.querySelectorAll(".scroll-track-visible"),
                { overflowBehavior: { x: "hidden", y: "scroll" } }
            );
            OverlayScrollbars(
                document.querySelectorAll(".scroll-horizontal-track-visible"),
                {
                    overflowBehavior: { x: "scroll", y: "hidden" },
                }
            );
    ...
    _initDropdownAsSelect() {
        ...
        document
            .querySelectorAll(".dropdown-as-select .dropdown-menu a")
            .forEach((element) => {
                element.addEventListener("click", (event) => {
                    ...
                    if (parent.getAttribute("data-childSelector")) {
                        childSelector =
                            " " + parent.getAttribute("data-childSelector");
                    }
                    parent.querySelector(
                        '[data-bs-toggle="dropdown"]' + childSelector
                    ).innerHTML = selText;
        ...
        document.querySelectorAll(".dropdown-as-select").forEach((element) => {
            ...
            if (element.getAttribute("data-childSelector")) {
                childSelector =
                    " " + element.getAttribute("data-childSelector");
            }
    ...
    _initModalPadding() {
        ...
        const paddingModal = new bootstrap.Modal(myModalEl, {
            backdrop: false,
        });
        ...
        document.querySelectorAll(".modal").forEach((el) => {
            ...
            el.addEventListener("show.bs.modal", function (event) {
                const rightPadding =
                    document.body.getAttribute("data-bs-padding");
                if (
        ...
        if (jQuery().validate) {
            jQuery.validator.setDefaults({
                ...
                errorPlacement: function (error, element) {
                    ...
                    if (
                        element.attr("class").indexOf("form-check-input") != -1
                    ) {
                    ...
                    if (element.parents(".tooltip-label-end").length > 0) {
                        if (
                            element.parents(".form-group").find(".form-label")
                                .length > 0
                        ) {
